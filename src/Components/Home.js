...
function Home({ allUser, setAllUser }) {
	...
	const [logger, setLogger] = useState({
		...
		Update_clicked: 0,
		Cancle_clicked: 0,
		Updated_card_userName: [],
		Deleted_card_username: [],
	});
	...
	const hendleclick = (User, resetForm) => {
		if (User.id !== 0) {
			...
			User.id = allUser[allUser.length - 1].id + 1;
	...
	const userValidatioon = Yup.object().shape({
		name: Yup.string().min(3, "It is too short").max(50, "It is too long").required("Name is Required"),
		password: Yup.string()
			...
			<Grid
				...
				<Paper
					...
					style={{
						padding: "20px",
						border: "15px solid",
						borderColor: "#006bb9 #30A0E0 #FFC973 #FFE3B3",
						borderRadius: "15px",
					}}
					noValidate
					autoComplete="off">
					{/* Why use h1 instead of typography */}
					<h1 style={{ textAlign: "center", fontSize: "xx-large" }}>
						{formik.values.id === 0 ? "Add new user" : "Update existing user"}
					</h1>
					...
					<OutlinedInput
						...
						endAdornment={
							<InputAdornment position="end">
								<IconButton onClick={handleshow}>{visibility ? <Visibility /> : <VisibilityOffIcon />}</IconButton>
							</InputAdornment>
						}
					/>
					...
					<Grid style={{ display: "flex", flexFlow: "row wrap", justifyContent: "space-around" }}>
						<FormControlLabel
							control={
								<Checkbox
									id="monday"
									checked={formik.values.days.monday}
									name="days.monday"
						...
						<FormControlLabel
							control={
								<Checkbox
									id="tuesday"
									checked={formik.values.days.tuesday}
									name="days.tuesday"
									onClick={formik.handleChange}
								/>
							}
						...
						<FormControlLabel
							control={
								<Checkbox
									id="thursday"
									checked={formik.values.days.thursday}
									name="days.thursday"
									onClick={formik.handleChange}
								/>
							}
						...
						<FormControlLabel
							control={
								<Checkbox
									id="friday"
									checked={formik.values.days.friday}
									name="days.friday"
									onClick={formik.handleChange}
								/>
							}
						...
						<FormControlLabel
							control={
								<Checkbox
									id="saturday"
									checked={formik.values.days.saturday}
									name="days.saturday"
									onClick={formik.handleChange}
								/>
							}
						...
						<FormControlLabel
							control={
								<Checkbox
									id="sunday"
									checked={formik.values.days.sunday}
									name="days.sunday"
									onClick={formik.handleChange}
								/>
							}
